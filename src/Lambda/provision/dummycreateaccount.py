...
def createAccount(accountemail,accountname,username,securityemail):
    ...
    executionArn = '{}-{}'.format(uuidname,str(uuid.uuid4()))
    return("createNewAccount:"+executionArn)
    
def accountStatus(operation_id):
    randaccountid="005765539777"
    #for x in range(0,12):
    #    randaccountid+=str(secrets.randbelow(9))
    response="SUCCESS:{}".format(randaccountid)
    return(response)
